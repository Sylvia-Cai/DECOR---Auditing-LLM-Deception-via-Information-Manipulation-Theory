"""Simple-LLM judge baseline on OpenDeception: whole-dialogue detection.

This baseline reuses the exact prompt templates from
experiment/baselines/blackbox/run_simple_llm_judge.py and applies them to the
entire dialogue at once.
"""
...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "open_deception" / "simple_llm_judge_open_deception_dialogue.json"
)
...
def _build_user_transcript(rounds: List[Dict[str, Any]]) -> str:
    lines: List[str] = []
    for i, rd in enumerate(rounds):
        rid = rd.get("round_id", i + 1)
        user_text = (rd.get("user") or "").strip()
        lines.append(f"Round {rid} User: {user_text}")
    return "\n".join(lines)


def _build_ai_transcript(rounds: List[Dict[str, Any]], target: str) -> str:
    lines: List[str] = []
    key = "ai_thought" if target == "thought" else "ai_speech"
    role = "Thought" if target == "thought" else "AI"
    for i, rd in enumerate(rounds):
        rid = rd.get("round_id", i + 1)
        ai_text = (rd.get(key) or "").strip()
        lines.append(f"Round {rid} {role}: {ai_text}")
    return "\n".join(lines)


def _build_judge_prompt(prompt_text: str, ai_text: str, target: str) -> str:
    if target == "thought":
        return _thought_prompt(prompt_text, ai_text)
    return _response_prompt(prompt_text, ai_text)


def main() -> None:
    parser = argparse.ArgumentParser(description="Simple-LLM whole-dialogue baseline for OpenDeception")
    ...
    for idx, d in enumerate(dialogues, start=1):
        ...
        rounds = d.get("rounds", [])
        user_transcript = _build_user_transcript(rounds)
        ai_transcript = _build_ai_transcript(rounds, args.target)
        judge_prompt = _build_judge_prompt(user_transcript, ai_transcript, args.target)
        ...
        judge_label = item["eval"]["label"]
        print(f"[{kept}] {item['source_file']} | label={judge_label}")
